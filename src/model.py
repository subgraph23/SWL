...
class Atom(nn.Module):
    ...
    def __init__(self, dim:int, max_dis:int, encode:bool=True):
        ...
        if encode:
            print('atom embedding parameters:', [param.numel() for param in self.embed_v.parameters()])
...
class Bond(nn.Module):
    ...
    def __init__(self, dim:int):
        ...
        print('bond embedding parameters:', [param.numel() for param in self.embed.parameters()])
    ...
    class _(nn.Module):
        ...
        def __init__(self, idim:int, odim:int):
            ...
            self.eps = nn.ParameterDict({
                agg: nn.Parameter(torch.zeros(1))
                for agg in scheme})

            self.enc = nn.ModuleDict({
                agg: Bond(idim)
                for agg in scheme if "L" in agg})
        ...
        def aggregate(self, agg, batch):
            return self.mlp[agg]((batch.x * (1.0 + self.eps[agg]) if gin else 0.0) + \
                                      aggregate(batch, agg, self.enc[agg]
                                      if agg in self.enc else None))

        def forward(self, batch):
            batch.x = self.nr(sum(
                self.aggregate(agg, batch) for agg in scheme))
            return batch
